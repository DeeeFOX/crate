...
public class CreateAlterTableStatementAnalyzerTest extends CrateDummyClusterServiceUnitTest {
    ...
    @Test
    public void testAlterTableDropCheckConstraint() throws Exception {
        SQLExecutor.builder(clusterService)
            .addTable("create table t (" +
                      "    id int primary key, " +
                      "    qty int constraint check_qty_gt_zero check(qty > 0), " +
                      ...
                      ")")
            .build();
        analyze("alter table t drop constraint check_qty_gt_zero");
        expectedException.expectMessage(
            "Cannot find a CHECK CONSTRAINT named [check_id_ge_zero_what], available names are: [check_id_ge_zero, check_qty_gt_zero]");
        analyze("alter table t drop constraint check_id_ge_zero_what");
    }
